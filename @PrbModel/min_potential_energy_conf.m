function [jointAngles, hessian, lambda, exitflag] = ...
    min_potential_energy_conf(obj, initialGuess, currents, externalWrenches, initialJointAngles, options)
%
...
%
    ...
    if isempty(obj.surface)
        ...
        constraint = @(jointAngles)surface_constraint(obj, jointAngles);
    end
...
function [c, ceq] = surface_constraint(obj, jointAngles)
    ceq = [];
    c = obj.surface.distance(obj.tip_position(jointAngles));  % Tip cannot go through surface
end
