...
[J_cu, J_ctheta, J_cq, J_tz]  = catheter.compute_contact_jacbobian(state, control, F_e, J_e, disturbances);

N_k = null(J_e);
dv = zeros(1,size(N_k, 2))';

dfc1 = 0.001* f_c(1) / sqrt(f_c(1)^2 + f_c(2)^2);
dfc2 = 0.001 * f_c(2) / sqrt(f_c(1)^2 + f_c(2)^2);
dfc = [dfc1, dfc2 , 0]';
J_cq
J_q
J_test = J_ctheta * J_q + J_cu
pinv(J_test) 
du = pinv(J_test) * dfc 
% state + N_k * J_cq * du
tip_ = catheter.tip_position(state + J_cq * du)
% du = 0.01*randn(6,1);
% % du= pinv(J_ctheta * J_q) * dfc;
% % du = pinv(J_cu) * dfc;
% 
control = control + du;
% state = state + J_q * du
